...
class IncomeController extends Controller
{
  ...
  public function getIncomeStatistics(Request $request)
  {
    ...
    $result = Income::query()
      ...
      ->selectRaw('sum((total - coalesce(discount, 0)) * stock_amount) as total, extract(month FROM payment_date) as month, extract(year FROM payment_date) as year')
      ->where('payment_date', '>=', $monthParam)
      ->groupByRaw('extract(month FROM payment_date), extract(year FROM payment_date)')
      ->get();
