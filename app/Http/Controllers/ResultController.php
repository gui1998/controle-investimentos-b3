...
class ResultController extends Controller
{
  ...
  public function getListResults(Request $request)
  {
    ...
    $result = Result::query()
      ...
      ->selectRaw('sum(value) as total, extract(month FROM result_date) as month, extract(year FROM result_date) as year')
      ->where('result_date', '>=', Carbon::now()->subMonths(6))
      ->groupByRaw('extract(month FROM result_date), extract(year FROM result_date)')
      ->get();
