...
class OperationController extends Controller
{
  ...
  public function store(Request $request, Operation $operation)
  {
    ...
    (new CreateResultsAction())->execute($operationCreated);
    (new CreateOrUpdateInvestmentAction())->execute($operationCreated);
  ...
  public function getBuyAndSellStatistics(Request $request)
  {
    ...
    $result = Operation::query()
      ...
      ->selectRaw('sum(price*stock_amount) as total, buy_r_sell, extract(month FROM operation_date) as month, extract(year FROM operation_date) as year')
      ->where('operation_date', '>=', Carbon::now()->subMonths(6))
      ->groupByRaw('extract(month FROM operation_date), buy_r_sell, extract(year FROM operation_date)')
  ...
  public function getIrrfStatistics(Request $request)
  {
    ...
    $result = Operation::query()
      ->where('user_id', $request->user('web')->id)
      ->selectRaw('sum(irrf*stock_amount) as total, extract(month FROM operation_date) as month, extract(year FROM operation_date) as year')
      ->where('operation_date', '>=', Carbon::now()->subMonths(6))
      ->groupByRaw('extract(month FROM operation_date), extract(year FROM operation_date)')
      ->get();
